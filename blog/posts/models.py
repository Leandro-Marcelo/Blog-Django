...
class Post(models.Model):
  ...
  created_at = models.DateField(default=timezone.now) # DateTimeField guarda tanto la fecha como la hora y DateField solo la fecha. Lo que haría Django es ejecutar este método de timezone.now cada vez que creemos un nuevo campo de fecha. Timezone es útil cuando queremos modificarlo, por ejemplo, fecha de entrega sería fecha de hoy mas 7 días y modificamos el valor por defecto, pero otra forma de realizarlo es auto_now=, ahora el auto_now se utiliza para que cada vez que se actualiza hace como el cambio, auto_now_add= es simplemente cuando se crea el elemento. Entonces podemos elegir entre default=timezone.now ó auto_now_add=True

  def __str__(self):
    """ convertir a un string pero con un formato de tiempo """
